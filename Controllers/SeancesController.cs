...
public class SeancesController : Controller
{
    ...
    public async Task<IActionResult> Create(Seance seance)
    {
        ...
        // Vérifie que le groupe d'étudiants (même Parcours + même Niveau) n'a pas déjà cours sur ce créneau
        bool conflitClasse = false;
        var coursActuel = await _context.Cours.FindAsync(seance.CoursId);
        if (coursActuel != null && coursActuel.ParcoursId != null)
        {
            var candidatsClasse = await _context.Seances
                .Include(s => s.Cours)
                .Where(s => s.Jour == seance.Jour
                         && s.Cours!.ParcoursId == coursActuel.ParcoursId
                         && s.Cours!.Niveau == coursActuel.Niveau)
                .ToListAsync();
            conflitClasse = candidatsClasse.Any(s => CreneauxSeChevauchent(s.HeureDebut, s.HeureFin, seance.HeureDebut, seance.HeureFin));
        }

        if (conflit || indisponible || conflitClasse)
        {
            if (conflit)
            ...
            if (conflitClasse)
                ModelState.AddModelError("", "👥 Alerte Conflit : ce groupe d'étudiants (même Parcours + même Niveau) a déjà un cours sur ce créneau !");
    ...
    public async Task<IActionResult> Edit(int id, Seance seance)
    {
        ...
        if (coursActuel != null && coursActuel.ParcoursId != null)
        {
            var candidatsClasse = await _context.Seances
                .Include(s => s.Cours)
                .Where(s => s.Id != seance.Id && s.Jour == seance.Jour
                         && s.Cours!.ParcoursId == coursActuel.ParcoursId
                         && s.Cours!.Niveau == coursActuel.Niveau)
                .ToListAsync();
            conflitClasse = candidatsClasse.Any(s => CreneauxSeChevauchent(s.HeureDebut, s.HeureFin, seance.HeureDebut, seance.HeureFin));
        ...
        if (ModelState.IsValid && !conflit && !indisponible && !conflitClasse)
        {
            _context.Update(seance);
            await _context.SaveChangesAsync();
    ...
    // Affiche l'emploi du temps d'une classe (Parcours + Niveau) sous forme de grille imprimable,
    // comme un emploi du temps papier classique (jours en colonnes, heures en lignes).
    [HttpGet]
